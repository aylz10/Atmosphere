...
namespace ams::dmnt::cheat::impl {
    ...
    namespace {
        ...
        constinit char g_text_file_buffer[512_KB];
        ...
        class CheatProcessManager {
            ...
            public:
                ...
                os::NativeHandle HookToCreateApplicationProcess() const {
                    os::NativeHandle h = os::InvalidNativeHandle;
                    const auto rc = pm::dmnt::HookToCreateApplicationProcess(std::addressof(h));
                    if (rc.IsFailure()) {
...
#define BP_Token proc_addr >> 56  
        ...
        void CheatProcessManager::DetectLaunchThread(void *_this) {
            ...
            AMS_DMNT2_DEBUG_LOG("[dmnt2:cheat] DetectLaunchThread started.\n");
            Event hook;
            while (true) {
                os::NativeHandle h = manager->HookToCreateApplicationProcess();
                if (h == os::InvalidNativeHandle) {
                    AMS_DMNT2_DEBUG_LOG("[dmnt2:cheat] HookToCreateApplicationProcess returned InvalidNativeHandle, retrying in 5 seconds...\n");
                    os::SleepThread(TimeSpan::FromSeconds(5));
                    continue;
                }
                AMS_DMNT2_DEBUG_LOG("[dmnt2:cheat] HookToCreateApplicationProcess successfully got handle: %d. Waiting for signal...\n", h);
                eventLoadRemote(std::addressof(hook), h, true);
                if (R_SUCCEEDED(eventWait(std::addressof(hook), std::numeric_limits<u64>::max()))) {
                    AMS_DMNT2_DEBUG_LOG("[dmnt2:cheat] Received process launch signal, calling AttachToApplicationProcess(true)...\n");
                    Result rc = manager->AttachToApplicationProcess(true);
                    AMS_DMNT2_DEBUG_LOG("[dmnt2:cheat] AttachToApplicationProcess(true) completed, result: 0x%08x\n", rc.GetValue());
                }
        ...
        void CheatProcessManager::DebugEventsThread(void *_this) {
            ...
            while (true) {
                ...
                while (true) {
                    ...
                    /* WaitSynchronization failed or timed out. This means someone canceled our synchronization, possibly us. */
        ...
        Result CheatProcessManager::AttachToApplicationProcess(bool on_process_launch) {
            AMS_DMNT2_DEBUG_LOG("[dmnt2:cheat] AttachToApplicationProcess(%d) called\n", on_process_launch);
            std::scoped_lock lk(m_cheat_lock);

            /* Close the active process, if needed. */
            {
                if (this->HasActiveCheatProcess()) {
                    AMS_DMNT2_DEBUG_LOG("[dmnt2:cheat] Already has active cheat process, on_process_launch: %d\n", on_process_launch);
            ...
            Result rc_pid = pm::dmnt::GetApplicationProcessId(std::addressof(m_cheat_process_metadata.process_id));
            AMS_DMNT2_DEBUG_LOG("[dmnt2:cheat] GetApplicationProcessId() result: 0x%08x, PID: %ld\n", rc_pid.GetValue(), m_cheat_process_metadata.process_id.value);
            R_ABORT_UNLESS_IF_NEW_PROCESS(rc_pid);

            auto proc_guard = SCOPE_GUARD {
                AMS_DMNT2_DEBUG_LOG("[dmnt2:cheat] proc_guard triggered (detaching / bailing out)\n");
                if (on_process_launch) {
            ...
            /* Get process handle, use it to learn memory extents. */
            {
                ...
                Result rc_proc = pm::dmnt::AtmosphereGetProcessInfo(std::addressof(proc_h), std::addressof(loc), std::addressof(status), m_cheat_process_metadata.process_id);
                AMS_DMNT2_DEBUG_LOG("[dmnt2:cheat] AtmosphereGetProcessInfo() result: 0x%08x, program_id: 0x%016lx, IsCheatEnabled: %d\n", rc_proc.GetValue(), loc.program_id.value, status.IsCheatEnabled());
                R_ABORT_UNLESS_IF_NEW_PROCESS(rc_proc);
                ...
                if (on_process_launch) {
                    if (!status.IsCheatEnabled()) {
                        AMS_DMNT2_DEBUG_LOG("[dmnt2:cheat] Cheats are NOT enabled for this process, aborting attachment.\n");
            ...
            /* Get module information from loader. */
            {
                ...
                /* TODO: ldr::dmnt:: */
                Result rc_mod = ldrDmntGetProcessModuleInfo(static_cast<u64>(m_cheat_process_metadata.process_id), reinterpret_cast<LoaderModuleInfo *>(proc_modules), util::size(proc_modules), std::addressof(num_modules));
                AMS_DMNT2_DEBUG_LOG("[dmnt2:cheat] ldrDmntGetProcessModuleInfo() result: 0x%08x, num_modules: %d\n", rc_mod.GetValue(), num_modules);
                R_ABORT_UNLESS_IF_NEW_PROCESS(rc_mod);
                ...
                if (num_modules == 2) {
                    ...
                    AMS_DMNT2_DEBUG_LOG("[dmnt2:cheat] Invalid number of modules (%d) for auto-attach: %d, aborting.\n", num_modules, on_process_launch);
                    R_THROW(dmnt::cheat::ResultCheatNotAttached());
                }
            ...
            bool loaded_cheats = this->LoadCheats(m_cheat_process_metadata.program_id, m_cheat_process_metadata.main_nso_module_id);
            AMS_DMNT2_DEBUG_LOG("[dmnt2:cheat] LoadCheats() result: %d\n", loaded_cheats);
            if (!loaded_cheats ||
            ...
            AMS_DMNT2_DEBUG_LOG("[dmnt2:cheat] AttachToApplicationProcess successfully finished attachment! Signal sent.\n");
            R_SUCCEED();
        }
    ...
    os::NativeHandle GetSharedDebugHandle() {
        return util::GetReference(g_cheat_process_manager).GetCheatProcessHandle();
    }
